...
uint256 hashGenesisBlock("0xed9b455f8ee0a9e631070e3b2ffec63f0411d6588b00f3b211ba07730e0ac88f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Abercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Abercoin Signed Message:\n";
...
{
    // Abercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Abercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Abercoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Abercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Abercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Abercoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Some message here; do this when the coin is ready";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 31337 << CBigNum(31337) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nVersion = 1;
        block.nTime    = 1394212341;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3133731337;

        if (fTestNet)
        {
            block.nTime    = 1394139736;
            block.nNonce   = 1385270584;
        }

        //// debug print
        uint256 hash = block.GetHash();
        printf("hash.ToString(): %s\n", hash.ToString().c_str());
        printf("hashGenesisBlock.ToString(): %s\n", hashGenesisBlock.ToString().c_str());
        printf("block.hashMerkleRoot.ToString(): %s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x93dd871329b25ebd85b7f0b1ddf0199d852da4ba219366688f9776dbc75a2d4e"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Abercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AbercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AbercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AbercoinMiner(CWallet *pwallet)
{
    printf("AbercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("abercoin-miner");
    ...
    try { loop {
        ...
        printf("Running AbercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AbercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AbercoinMiner, pwallet));
}
